...
class ViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if(DataProvider.shouldChange == true){
            
            //Array of all the buttons
            let buttons: [UIButton] = [self.b1️⃣, self.b2️⃣, self.b3️⃣ ,self.b4️⃣, self.b5️⃣, self.b6️⃣, self.b7️⃣, self.b8️⃣, self.b9️⃣, self.b0️⃣, self.bPlus, self.bMinus, self.bMult, self.bDiv, self.bEq, self.bClear]
           
            
            //If we should change the background color
            if(DataProvider.shouldChangeBackground == true){
                view.backgroundColor = UIColor(red: DataProvider.passed_Red_Background_Value, green: DataProvider.passed_Green_Background_Value, blue: DataProvider.passed_Blue_Background_Value, alpha: 1.0)
            }
            
            //Loop through all the buttons and change their tint
            for button in buttons {
                if(DataProvider.shouldChangeTint == true){ //If we should change their tint
                    button.tintColor = UIColor(red: DataProvider.passed_Red_Tint_Value, green: DataProvider.passed_Green_Tint_Value, blue: DataProvider.passed_Blue_Tint_Value, alpha: 1.0)
                }
            }
            
            //If we should change the text color of the displayLabel
            if(DataProvider.shouldChangeText == true){
                displayLabel.textColor = UIColor(red: DataProvider.passed_Red_Text_Value, green: DataProvider.passed_Green_Text_Value, blue: DataProvider.passed_Blue_Text_Value, alpha: 1.0)
            }
            
            
            DataProvider.shouldChange = false
            DataProvider.shouldChangeBackground = false
            DataProvider.shouldChangeTint = false
            DataProvider.shouldChangeText = false
        }
    }
